...
// ─── AUTOCOMPLETE (Scryfall live API) ────────────────────────────────────────
// Detects partner mechanics from oracle text / keywords
const PARTNER_KEYWORDS = [
  'partner with', 'partner', 'friends forever',
  'choose a background', "doctor's companion"
];

function hasPartner(card) {
  const kwArr = card.keywords || [];
  const oracle = (card.oracle_text || '').toLowerCase();
  for (const kw of kwArr) {
    if (PARTNER_KEYWORDS.includes(kw.toLowerCase())) return true;
  }
  for (const kw of PARTNER_KEYWORDS) {
    if (oracle.includes(kw)) return true;
  }
  return false;
}
...
async function searchCommanders(query) {
  const q = query.trim();
  if (scryfallCache[q]) return scryfallCache[q];
  try {
    const encoded = encodeURIComponent(`"${q}" is:commander legal:commander`);
    const url = `https://api.scryfall.com/cards/search?q=${encoded}&order=name&unique=cards`;
    const res = await fetch(url);
    if (!res.ok) {
      // fallback: autocomplete endpoint (names only, no metadata)
      return await searchCommandersAutocomplete(q);
    }
    const data = await res.json();
    const results = (data.data || []).slice(0, 12).map(card => ({
      name: card.name,
      colors: getColorIdentity(card),
      partner: hasPartner(card),
      card_faces: card.card_faces || null,
    }));
    scryfallCache[q] = results;
    return results;
  } catch(e) {
    return await searchCommandersAutocomplete(q);
  }
...
async function searchCommandersAutocomplete(query) {
  try {
    const encoded = encodeURIComponent(query);
    const url = `https://api.scryfall.com/cards/autocomplete?q=${encoded}&include_extras=false`;
    const res = await fetch(url);
    const data = await res.json();
    return (data.data || []).slice(0, 12).map(name => ({
      name,
      colors: '?',
      partner: false,
    }));
...
function setupCmdrAC(inputId, dropdownId, which) {
  ...
  const doSearch = debounce(async () => {
    ...
    // Show loading state
    dropdown.innerHTML = '<div class="autocomplete-item" style="color:var(--muted);font-style:italic;">Buscando...</div>';
    dropdown.classList.add('open');

    const results = await searchCommanders(val);

    if (!results.length) {
      dropdown.innerHTML = '<div class="autocomplete-item" style="color:var(--muted);">Sin resultados</div>';
      return;
    }

    dropdown.innerHTML = results.map(c => {
      const colorStr = c.colors === '?' ? '' : c.colors;
      const pips = colorStr === 'C'
        ? '<span class="ci-pip" style="background:var(--surface2);border:1px solid var(--border-bright);"></span>'
        : [...colorStr].map(col => `<span class="ci-pip ci-${col}"></span>`).join('');
      const partnerBadge = c.partner
        ? ' <small style="color:var(--gold);font-size:0.63rem;margin-left:4px;">[Partner]</small>' : '';
      const colorData = colorStr || 'C';
      return `<div class="autocomplete-item"
        data-name="${c.name.replace(/"/g,'&quot;')}"
        data-colors="${colorData}"
        data-partner="${c.partner}"
        onclick="selectCommander(this, ${which})">
...
function selectCommander(el, which) {
  ...
  const colors = el.dataset.colors || 'C';
  ...
  if (which === 1) {
    ...
    // enable second commander slot only if has partner mechanic
    const c2 = document.getElementById('cmdr2-input');
    c2.disabled = !partner;
    if (!partner) {
      c2.value = '';
      selectedCmdr2 = null;
      document.getElementById('cmdr2-color-pills').innerHTML = '';
    }
  } else {
...
function renderColorPills(containerId, colorStr) {
  ...
  if (!colorStr || colorStr === 'C' || colorStr === '?') {
    container.innerHTML = '<span class="color-pill C">Incoloro</span>';
    return;
  }
